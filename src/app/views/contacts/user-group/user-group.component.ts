...
export class UserGroupComponent implements OnInit {
  ...
  groupId: any;  //组ID
  isGroupType: boolean = false; //宽度是否是企业群组 false 企业群组，true个人群组
  ...
  // 获取我的群组
  getUserGroupList() {
    this.http.get('/uc/userGroup/groups?groupname=' + this.searchName).subscribe(
      res => {
        let datalist: any = res;
        if (datalist.code == 200) {
  ...
  getUserGroupOther() {
    this.http.get('/uc/userGroup/other/user/' + this.groupId).subscribe(
      res => {
        let datalist: any = res;
        if (datalist.code == 200) {
  ...
  getContactsList() {
    ...
    this.http.post('/uc/userGroup/entuser/inGroup', getData).subscribe(
      res => {
        let datalist: any = res;
        if (datalist.code == 200) {
  ...
  saveAddGroupFn() {
    ...
    this.http.post('/uc/userGroup/user', getData).subscribe(
      res => {
        let datalist: any = res;
        if (datalist.code == 200) {
          // this.getUserGroup();
